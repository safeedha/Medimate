...
import { geteverymessage, deletMessage,Messagetimeadding} from '../../api/userapi/chat';
...
const chat = messages.map((item, index) => {
  ...
  return (
    ...
    >
      <div className="flex flex-col items-start max-w-xs relative z-0">
      
        <div className="relative">
          {item?.image && (
            <div className="relative group mb-1 max-w-[200px] max-h-[200px]">
              <img
                src={`https://res.cloudinary.com/dwerqkqou/image/upload/${item.image}`}
                alt="sent"
  ...
  return (
    <div className="py-4 px-4 h-[calc(100vh-4rem)]">
      <div className="flex flex-col h-full border border-gray-300 rounded-md relative z-0">
        <Toaster />
        <div className="flex items-center justify-between p-4 border-b border-gray-300 bg-gray-100 z-10 relative">
          <div className="flex items-center gap-2">
            <a href={`/video/${roomId}?role=user&userId=${user?._id}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:text-blue-700">
        ...
        <div className="flex-1 overflow-y-auto p-4 relative z-0 min-h-0">
          {chat}
          ...
          <div className="p-2 border-t border-gray-300 bg-gray-50 flex justify-between items-center">
            ...
            {!isUploading && (
              <button onClick={() => {
                setFile(null);
                setImageurl(null);
              }} className="text-red-500 hover:underline text-sm">Remove</button>
            )}
          </div>
        )}
        ...
        <div className="relative flex items-center gap-2 border-t border-gray-200 p-4">
          ...
          <input type="text" placeholder="Type your message..." value={message} onChange={(e) => setMessage(e.target.value)} className="flex-1 px-3 py-2 border rounded-md border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-400" />
          <button onClick={handleSend} disabled={(!message.trim() && !imageurl) || isUploading} className={`px-4 py-2 text-white rounded-md transition ${(!message.trim() && !imageurl) || isUploading ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-600'}`}>
