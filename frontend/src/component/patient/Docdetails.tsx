...
function Docdetails() {
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    e.preventDefault()
    if (!formData.fullName || !formData.age || !formData.gender || !formData.email || !formData.phone || !formData.reason || !formData.paymentMethod) {
...
>
  <div className="flex flex-col md:flex-row w-full p-3 gap-3">
    ...
    <div className="md:w-1/2 bg-gray-50 p-3 rounded shadow-sm text-xs text-gray-700">
      <div className="bg-slate-200 p-2 rounded mb-2">
        ...
        <p className="text-gray-600">
          Specialisation: <span className="font-semibold text-green-700">{doctor?.specialisation?.deptname}</span>
        </p>
        <p className="text-gray-600">
          Consultation Fee: <span className="font-semibold text-green-700">₹{doctor?.fee}</span>
        </p>
        <p className="text-gray-600">
          Consultation Time: <span className="font-medium text-blue-600">{selectedSlot?.startingTime} - {selectedSlot?.endTime}</span>
        </p>
      </div>

      <h2 className="text-base font-semibold mb-1">Privacy Policy</h2>
      <p className="mb-1">
        Your personal data is collected and used solely for booking and managing your
        appointment. We ensure confidentiality and never share your information with third
        parties without consent.
      </p>
      <p className="mb-1">
        By submitting this form, you agree to our terms of use and consent to being contacted
        regarding your appointment.
      </p>
      <p>
        If you have any questions about how we handle your data, please contact our support
        team.
      </p>
    </div>
    ...
    >
      <h2 className="text-base font-semibold mb-1">Patient Details</h2>
      ...
      <div className="bg-white shadow-md rounded-md p-4 mt-6 mx-auto max-w-5xl">
        ...
        <div className="bg-teal-50 p-4 rounded-md shadow-inner">
          <h2 className="text-sm font-medium text-gray-700 mb-2">Available Time Slots:</h2>
          {slot.length > 0 ? (
